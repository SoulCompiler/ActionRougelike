...
void ASCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("Turn", this, &APawn::AddControllerYawInput);		// 将改变Controller偏航角（物体坐标的偏航角）的函数绑定到Turn的键。该函数是基类Pawn中自带的虚函数。
...
void ASCharacter::MoveRight(float Value)
{
	...
	FVector RightVector = FRotationMatrix(ControlRot).GetScaledAxis(EAxis::Y);		// 不知道RightVector怎么获取时，去蓝图中搜索，然后根据蓝图显示的信息再在VS中使用VA的Find Symbol功能搜索该变量相关的名字RightVector，找到和蓝图信息相符的函数，就可以获取正确的变量。
...
void ASCharacter::PrimaryAttack()
{
	...
	GetWorldTimerManager().SetTimer(TimerHandle_PrimaryAttack,this,&ASCharacter::PrimaryAttack_TimeElapsed,0.2f);		// 将需要在播放动画延时中执行的函数放在Timer中。
...
void ASCharacter::PrimaryAttack_TimeElapsed()
{
	...
	FTransform SpawnTM = FTransform(GetControlRotation(), HandLocation);		// 第二个参数是投射物的出生点的位置

	FActorSpawnParameters SpawnParams;
	SpawnParams.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;		// 当Spawn出生点产生碰撞时总是SpawnActor
